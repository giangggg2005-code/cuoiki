...
public class RoomServiceImpl implements RoomService {
    ...
    @Override
    public boolean updateRoomPrice(int roomId, double roomPrice) {
        if (roomId <= 0) {
            throw new RoomException("Mã phòng không hợp lệ.");
        }
        if (roomPrice < 0) {
            throw new RoomException("Tiền phụ thu phòng không được âm.");
        }
        try {
            Room existing = roomRepo.findById(roomId);
            if (existing == null) {
                throw new RoomException("Không tìm thấy phòng chiếu cần cập nhật giá.");
            }
            existing.setRoomPrice(roomPrice);
            return roomRepo.update(existing);
        } catch (DataAccessException e) {
            throw new RoomException("Lỗi hệ thống cơ sở dữ liệu: Không thể kết nối hoặc thực thi truy vấn!");
        }
    }

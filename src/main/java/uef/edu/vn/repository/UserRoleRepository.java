...
public class UserRoleRepository {
    ...
    public boolean hasActiveAdminAccess(int userId) {
        return hasActiveRoleAccess(userId, "ADMIN");
    }

    public boolean hasActiveManagerAccess(int userId) {
        return hasActiveRoleAccess(userId, "MANAGER");
    }

    private boolean hasActiveRoleAccess(int userId, String roleName) {
        String sql = "SELECT COUNT(*) FROM `User` u " +
                     ...
                     "AND UPPER(q.roleName) = ?";
        Integer count = jdbcTemplate.queryForObject(sql, Integer.class, userId, roleName.toUpperCase());
        return count != null && count > 0;
    }

...
public class MovieRepository {
    ...
    public int updateBasePrice(int movieId, double basePrice) {
        String sql = "UPDATE `Movie` SET basePrice = ? WHERE id_Movie = ?";
        return jdbcTemplate.update(sql, basePrice, movieId);
    }
    ...
    public List<Movie> searchMoviesWithShowtimesIn7Days(String keyword, String genre) {
        StringBuilder sql = new StringBuilder(
                ...
                "INNER JOIN `Showtime` s ON m.id_Movie = s.movieId " +
        ...
        if (genre != null && !genre.trim().isEmpty() && !"all".equalsIgnoreCase(genre.trim())) {
    ...
    public List<Movie> findMoviesWithShowtimeOnDate(java.sql.Date date) {
        String sql = "SELECT DISTINCT m.* FROM `Movie` m " +
                     "INNER JOIN `Showtime` s ON m.id_Movie = s.movieId " +
                     "WHERE s.showDate = ? AND s.status = 'Active' " +

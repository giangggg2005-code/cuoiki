...
public class SeatRepository {
    ...
    public Seat findBySeatNameAndRoomId(String seatName, int roomId) {
        String sql = "SELECT * FROM `Seat` WHERE seatName = ? AND roomId = ?";
        try {
            return jdbcTemplate.queryForObject(sql, this::mapRow, seatName, roomId);
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    }

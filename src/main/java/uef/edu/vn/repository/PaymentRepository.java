...
public class PaymentRepository {
    ...
    public int saveAndGetId(Payment p) {
        String sql = "INSERT INTO `Payment` (userId, cardNumber, balance, pinCode) VALUES (?, ?, ?, ?)";
    ...
    /**
     * Tìm thẻ theo số thẻ (bỏ qua dấu gạch ngang, khoảng trắng khi so khớp)
     */
    public Payment findByNormalizedCardNumber(String normalizedDigits) {
        if (normalizedDigits == null || normalizedDigits.isEmpty()) {
            return null;
        }
        String sql = "SELECT * FROM `Payment` " +
                     "WHERE REPLACE(REPLACE(REPLACE(cardNumber, '-', ''), ' ', ''), '.', '') = ?";
        try {
            return jdbcTemplate.queryForObject(sql, this::mapRow, normalizedDigits);
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    }

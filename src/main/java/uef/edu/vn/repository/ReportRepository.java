...
public List<Map<String, Object>> getDetailedTransactions(String startDate, String endDate, Integer movieId, String roomType, int limit, int offset, String sortBy, String sortDir) {
  ...
  // 7. BIỂU ĐỒ DOANH THU PHIM THEO NGÀY (LẤY TẤT CẢ CÁC NGÀY CÓ SUẤT CHIẾU, KỂ CẢ DOANH THU = 0)
   ...
   // 8. THỐNG KÊ HIỆU SUẤT THEO TỪNG SUẤT CHIẾU (Đã fix lỗi SQL Grammar)
    ...
    public Map<String, Object> getTodayStaffSummary() {
        Map<String, Object> summary = new HashMap<>();
        try {
            String sqlTickets = "SELECT COUNT(bd.id_Detail) FROM `BookingDetail` bd " +
                                "JOIN `Booking` b ON bd.bookingId = b.id_Booking " +
                                "WHERE b.status = 'Completed' AND DATE(b.bookingDate) = CURRENT_DATE";
            Integer todayTickets = jdbcTemplate.queryForObject(sqlTickets, Integer.class);
            summary.put("todayTickets", todayTickets != null ? todayTickets : 0);

            String sqlRevenue = "SELECT SUM(totalAmount) FROM `Booking` " +
                                "WHERE status = 'Completed' AND DATE(bookingDate) = CURRENT_DATE";
            Double todayRevenue = jdbcTemplate.queryForObject(sqlRevenue, Double.class);
            summary.put("todayRevenue", todayRevenue != null ? todayRevenue : 0.0);

            String sqlCustomers = "SELECT COUNT(DISTINCT u.id_User) FROM `User` u " +
                                  "JOIN `UserRole` ur ON u.id_User = ur.id_User " +
                                  "JOIN `Quyen` q ON ur.id_Role = q.id_Role " +
                                  "WHERE q.roleName = 'CUSTOMER'";
            Integer totalCustomers = jdbcTemplate.queryForObject(sqlCustomers, Integer.class);
            summary.put("totalCustomers", totalCustomers != null ? totalCustomers : 0);

            String sqlShowtimes = "SELECT COUNT(*) FROM `Showtime` WHERE DATE(showDate) = CURRENT_DATE AND status != 'Cancelled'";
            Integer todayShowtimeCount = jdbcTemplate.queryForObject(sqlShowtimes, Integer.class);
            summary.put("todayShowtimeCount", todayShowtimeCount != null ? todayShowtimeCount : 0);

            return summary;
        } catch (Exception e) {
            throw new ReportException("Lỗi lấy tổng quan staff hôm nay: " + e.getMessage(), e);
        }
    }

    public List<Map<String, Object>> getTodayTicketSalesByMovie(int limit) {
        try {
            String sql = "SELECT m.id_Movie, m.title AS movieName, COUNT(bd.id_Detail) AS tickets " +
                         "FROM `Movie` m " +
                         "JOIN `Showtime` s ON m.id_Movie = s.movieId " +
                         "JOIN `Ticket` t ON s.id_Showtime = t.showtimeId " +
                         "JOIN `BookingDetail` bd ON t.id_Ticket = bd.ticketId " +
            ...
            throw new ReportException("Lỗi tải tỷ lệ vé theo phim hôm nay: " + e.getMessage(), e);
        }
    }

    public List<Map<String, Object>> getTodayRevenueByShift() {
        try {
            String sql = "SELECT CASE " +
                         "    WHEN HOUR(bookingDate) BETWEEN 6 AND 11 THEN 'Buổi Sáng' " +
                         "    WHEN HOUR(bookingDate) BETWEEN 12 AND 17 THEN 'Buổi Chiều' " +
                         ...
                         "FROM `Booking` " +
                         "WHERE status = 'Completed' AND DATE(bookingDate) = CURRENT_DATE " +
                         "GROUP BY shift ORDER BY FIELD(shift, 'Buổi Sáng', 'Buổi Chiều', 'Buổi Tối', 'Nửa Đêm')";
            return jdbcTemplate.query(sql, (rs, rowNum) -> {
                Map<String, Object> map = new HashMap<>();
                map.put("shift", rs.getString("shift"));
                map.put("bookings", rs.getInt("totalBookings"));
            ...
            throw new ReportException("Lỗi doanh thu theo ca hôm nay: " + e.getMessage(), e);
        }
    }

    public List<Map<String, Object>> getRecentBookingsToday(int limit) {
        try {
            String sql = "SELECT b.id_Booking, u.fullName AS customerName, m.title AS movieTitle, " +
                         "COUNT(bd.id_Detail) AS ticketCount, b.totalAmount AS amount, " +
                         "b.paymentMethod, b.bookingDate, b.status " +
                         "FROM `Booking` b " +
                         ...
                         "GROUP BY b.id_Booking, u.fullName, m.title, b.totalAmount, b.paymentMethod, b.bookingDate, b.status " +
                         "ORDER BY b.bookingDate DESC LIMIT ?";
            return jdbcTemplate.query(sql, (rs, rowNum) -> {
                Map<String, Object> map = new HashMap<>();
                map.put("bookingId", rs.getInt("id_Booking"));
                map.put("customerName", rs.getString("customerName") != null ? rs.getString("customerName") : "Khách vãng lai");
                map.put("movieTitle", rs.getString("movieTitle"));
                map.put("ticketCount", rs.getInt("ticketCount"));
                map.put("amount", rs.getDouble("amount"));
            ...
            throw new ReportException("Lỗi đơn đặt vé gần đây hôm nay: " + e.getMessage(), e);
        }
    }

    public List<Map<String, Object>> getShowtimesWithRoomByDate(String date) {
        try {
            String sql = "SELECT s.id_Showtime, s.movieId, m.title AS movieTitle, m.basePrice, m.posterUrl, " +
                         "r.roomName, s.showDate, s.startTime, s.status, " +
                         "(SELECT COUNT(*) FROM `Ticket` t WHERE t.showtimeId = s.id_Showtime AND t.status = 'Sold') AS bookedSeats, " +
                         "(r.totalRows * r.totalCols) AS totalSeats " +
                         "FROM `Showtime` s " +
                         "JOIN `Movie` m ON s.movieId = m.id_Movie " +
                         "JOIN `Room` r ON s.roomId = r.id_Room " +
                         "WHERE DATE(s.showDate) = ? AND s.status != 'Cancelled' " +
                         "ORDER BY s.startTime ASC";
            return jdbcTemplate.query(sql, (rs, rowNum) -> {
                Map<String, Object> map = new HashMap<>();
                map.put("showtimeId", rs.getInt("id_Showtime"));
                map.put("movieId", rs.getInt("movieId"));
                map.put("movieTitle", rs.getString("movieTitle"));
                map.put("basePrice", rs.getDouble("basePrice"));
                map.put("posterUrl", rs.getString("posterUrl"));
                map.put("roomName", rs.getString("roomName"));
                map.put("showDate", rs.getDate("showDate"));
                String time = rs.getString("startTime");
                map.put("startTime", time != null && time.length() >= 5 ? time.substring(0, 5) : time);
                map.put("status", rs.getString("status"));
                map.put("bookedSeats", rs.getInt("bookedSeats"));
                map.put("totalSeats", rs.getInt("totalSeats"));
                return map;
            ...
            throw new ReportException("Lỗi lịch chiếu theo ngày: " + e.getMessage(), e);
        }
    }
}

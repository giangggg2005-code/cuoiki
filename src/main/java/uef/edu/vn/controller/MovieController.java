...
public class MovieController {
   ...
   // Nhớ import thêm 2 thư viện này ở đầu file nhé:
    ...
    @PostMapping("/movies/update-base-price")
    public Object updateBasePrice(@RequestParam("id") int movieId,
            @RequestParam("basePrice") double basePrice,
            @RequestParam(value = "inline", required = false) String inline,
            RedirectAttributes redirectAttributes) {
        try {
            movieService.updateMovieBasePrice(movieId, basePrice);
            if ("1".equals(inline)) {
                Map<String, Object> body = new HashMap<>();
                body.put("success", true);
                body.put("message", "Cập nhật giá vé cơ bản thành công!");
                return ResponseEntity.ok(body);
            }
            redirectAttributes.addFlashAttribute("successMessage", "Cập nhật giá vé cơ bản thành công!");
        } catch (Exception e) {
            if ("1".equals(inline)) {
                Map<String, Object> body = new HashMap<>();
        ...
        return "redirect:/admin/movies/detail?id=" + movieId;
    }
}
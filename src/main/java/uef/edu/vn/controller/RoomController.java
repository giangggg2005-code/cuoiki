...
public class RoomController {
    ...
    public String roomDetail(@RequestParam("id") int id,
        ...
        model.addAttribute("portalRole", PortalSessionHelper.getPortalRole(session));
        model.addAttribute("view", "rooms");
        model.addAttribute("body", path + "room_detail.jsp");
        return pathview;
    }

    @PostMapping("/rooms/update-room-price")
    public Object updateRoomPrice(@RequestParam("id") int roomId,
            @RequestParam("roomPrice") double roomPrice,
            @RequestParam(value = "inline", required = false) String inline,
            RedirectAttributes redirectAttributes) {
        try {
            roomService.updateRoomPrice(roomId, roomPrice);
            if ("1".equals(inline)) {
                Map<String, Object> body = new HashMap<>();
                body.put("success", true);
                body.put("message", "Cập nhật tiền phụ thu phòng chiếu thành công!");
                return ResponseEntity.ok(body);
            }
            addSuccessMessage(redirectAttributes, "Cập nhật tiền phụ thu phòng chiếu thành công!");
        } catch (RoomException e) {
            if ("1".equals(inline)) {
                Map<String, Object> body = new HashMap<>();
            ...
            if ("1".equals(inline)) {
                Map<String, Object> body = new HashMap<>();
                body.put("success", false);
                body.put("message", "Lỗi hệ thống: " + e.getMessage());
                return ResponseEntity.badRequest().body(body);
            }
            addErrorMessage(redirectAttributes, "Lỗi hệ thống: " + e.getMessage());
        }
        return "redirect:/admin/rooms/detail?id=" + roomId + "&mode=edit";
    }

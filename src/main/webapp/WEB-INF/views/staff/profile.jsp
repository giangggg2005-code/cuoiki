...
<c:set var="displayFullName" value="${not empty formFullName ? formFullName : staff.fullName}" />
<c:set var="displayEmail" value="${not empty formEmail ? formEmail : staff.email}" />
<c:set var="displayPhone" value="${not empty formPhone ? formPhone : staff.phone}" />
<c:set var="displayAvatar" value="${not empty formAvatar ? formAvatar : staff.avatar}" />
...
<c:choose>
    ...
    <c:otherwise>
        <c:set var="previewSrc" value="https://ui-avatars.com/api/?name=${fn:replace(displayFullName, ' ', '+')}&background=121212&color=3b82f6&bold=true"/>
    </c:otherwise>
</c:choose>
...
<style>
    ...
    .toast-error-glow { animation: glowPulseBlue 3s infinite ease-in-out; border-color: rgba(239, 68, 68, 0.35) !important; }
...
<div class="max-w-4xl mx-auto space-y-8">
    <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
            ...
            <p class="text-gray-500 mt-2 text-sm">Cập nhật hồ sơ nhân viên của bạn.</p>
        </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-[#121212] border border-gray-800 rounded-2xl p-8">
                <div class="flex flex-col items-center text-center mb-6">
                    <div class="w-28 h-28 rounded-full p-[2px] bg-gradient-to-tr from-blue-600 to-cyan-500 mb-4">
                        <img id="staff-avatar-preview"
                             src="${previewSrc}"
                             alt="Avatar"
                             class="w-full h-full object-cover rounded-full bg-[#121212]"
                             onerror="this.src='https://ui-avatars.com/api/?name=${fn:replace(displayFullName, ' ', '+')}&background=121212&color=3b82f6&bold=true';">
                    </div>
                    <p class="text-white font-extrabold"><c:out value="${displayFullName}" /></p>
                    <p class="text-blue-500 text-xs font-bold mt-1 uppercase tracking-wider">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-white font-semibold"><c:out value="${staff.username}" /></span>
                    </div>
                    <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-green-400 font-semibold"><c:out value="${staff.status}" /></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-[#121212] border border-gray-800 rounded-2xl p-8">
                <h3 class="text-lg font-bold text-white mb-6 flex items-center gap-2">
                    <i class="fas fa-user-pen text-blue-500"></i> Chỉnh sửa hồ sơ
                </h3>

                <form id="staffProfileForm" action="${pageContext.request.contextPath}/staff/profile/update" method="post" enctype="multipart/form-data" class="space-y-5" novalidate>
                    <div>
                        <label for="fullName" class="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Họ và tên <span class="text-blue-500">*</span></label>
                        <input type="text" id="fullName" name="fullName" maxlength="100"
                               value="<c:out value='${displayFullName}' />"
                               class="input-staff profile-field"
                               placeholder="Nguyễn Văn A...">
                        <span id="error-fullName" class="text-[11px] text-red-400 mt-1 hidden"></span>
                    </div>

                    <div>
                        <label for="email" class="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Email <span class="text-blue-500">*</span></label>
                    ...
                    <div>
                        <label for="phone" class="block text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">Số điện thoại <span class="text-blue-500">*</span></label>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-3">
                            ...
                            <div class="sm:col-span-1">
                                ...
                                <button type="button" onclick="document.getElementById('avatarFile').click()"
                                        class="w-full py-3 px-4 bg-[#1a1a1a] hover:bg-[#222] text-gray-300 rounded-xl border border-gray-700 hover:border-blue-500/50 transition flex items-center justify-center gap-2 text-sm font-medium min-h-[48px]">
                    ...
                    <div class="pt-4 border-t border-gray-800 flex flex-wrap gap-4">
                        <button type="button" onclick="validateProfileForm()"
                                class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-xl text-sm uppercase tracking-widest transition-all">
...
<div id="customConfirmModal" class="fixed inset-0 bg-black/80 z-[10000] hidden items-center justify-center backdrop-blur-sm transition-opacity duration-300 opacity-0">
    <div id="confirmModalBox" class="bg-[#121212] border border-blue-500/50 rounded-2xl p-6 max-w-md w-full shadow-2xl transform scale-95 transition-all duration-300 mx-4">
        <div class="text-center mb-5">
            <div class="w-20 h-20 bg-blue-500/10 border-blue-500/30 rounded-full flex items-center justify-center mx-auto mb-4 border">
                <i class="fas fa-user-pen text-4xl text-blue-500"></i>
            </div>
            <h3 class="text-xl font-bold text-white mb-2 uppercase tracking-wider">Xác nhận cập nhật</h3>
            <p class="text-sm text-gray-400 leading-relaxed">Bạn có chắc chắn muốn <strong class="text-blue-400">lưu thông tin cá nhân</strong> mới không?</p>
        </div>
        <div class="flex gap-3">
            ...
            <button type="button" id="btn-confirm-submit" onclick="submitProfileForm()" class="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition">
...
<script>
    ...
    const fallbackAvatar = 'https://ui-avatars.com/api/?name=${fn:replace(displayFullName, " ", "+")}&background=121212&color=3b82f6&bold=true';
    ...
    function triggerPremiumToast(type, title, message) {
        ...
        const wrapperClass = isSuccess
            ...
            : (type === 'info'
                ? 'bg-zinc-950/90 text-white toast-info-glow border-blue-500/30'
                : 'bg-zinc-950/90 text-white toast-error-glow border-red-500/30');
        const titleColor = isSuccess ? 'text-green-400' : (type === 'info' ? 'text-blue-400' : 'text-red-400');
        const iconBg = isSuccess ? 'bg-green-500/10 border-green-500/20 text-green-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400';
        const barGradient = isSuccess ? 'from-green-500 via-emerald-400 to-teal-500' : 'from-blue-500 via-cyan-400 to-indigo-500';
    ...
    function submitProfileForm() {
        ...
        setTimeout(() => document.getElementById('staffProfileForm').submit(), 400);
    }
    ...
    function handleFileSelect(input) {
        if (input.files && input.files[0]) {
            ...
            document.getElementById('staff-avatar-preview').src = previewUrl;
        }
    }

    function previewAvatar() {
        const avatarUrl = document.getElementById('avatar').value.trim();
        const fullName = document.getElementById('fullName').value.trim() || 'Staff';
        const img = document.getElementById('avatar-preview-img');
        const profileImg = document.getElementById('staff-avatar-preview');
        const fileInput = document.getElementById('avatarFile');
        ...
        if (avatarUrl !== '') {
            ...
            const fallback = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(fullName) + '&background=121212&color=3b82f6&bold=true';
            img.src = fallback;

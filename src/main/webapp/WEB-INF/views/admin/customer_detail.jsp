...
<div class="mb-5">
    <c:if test="${readOnlyMode}">
        <div class="mb-4 p-4 rounded-xl border border-amber-500/30 bg-amber-950/20 text-amber-300 text-sm font-bold flex items-center gap-2">
            <i class="fas fa-eye"></i> Chế độ xem chỉ đọc — bạn không thể chỉnh sửa thông tin khách hàng.
        </div>
    </c:if>
    <a href="${pageContext.request.contextPath}${readOnlyMode ? '/admin/monitoring' : '/admin/customers'}" class="text-gray-400 hover:text-red-500 font-bold text-sm transition-colors duration-200 inline-flex items-center group">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('#formUpdateInfo input:not([type=hidden]), #formUpdateInfo textarea, #formUpdateInfo select, #formUpdateStatus input, #formUpdateStatus select').forEach(function (el) {
            el.readOnly = true;
            if (el.tagName === 'SELECT') el.disabled = true;
        });
        document.querySelectorAll('#formUpdateInfo button, #formResetPass button, #formUpdateStatus button').forEach(function (btn) {
            btn.style.display = 'none';
        });
        document.querySelectorAll('#formUpdateInfo, #formUpdateStatus, #formResetPass').forEach(function (form) {
            form.addEventListener('submit', function (e) {
                e.preventDefault();
                if (typeof triggerPremiumToast === 'function') {
                    triggerPremiumToast('error', 'Không có quyền', denyMsg);
                } else {
                    alert(denyMsg);
                }
            });

...
<c:set var="isViewMode" value="${mode == 'view' || readOnlyMode}" />
...
<c:if test="${readOnlyMode}">
    <div class="mb-4 p-4 rounded-xl border border-amber-500/30 bg-amber-950/20 text-amber-300 text-sm font-bold flex items-center gap-2">
        <i class="fas fa-eye"></i> Chế độ xem chỉ đọc — các thuộc tính phòng không thể chỉnh sửa; bạn chỉ có thể cập nhật tiền phụ thu bên dưới.
    </div>
</c:if>

<div class="mb-6 flex justify-between items-end">
    <div>
        <a id="btnBackToList" href="${pageContext.request.contextPath}${readOnlyMode ? '/admin/showtimes' : '/admin/rooms'}" class="text-gray-400 hover:text-red-500 transition text-sm flex items-center mb-2 font-bold group">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div class="bg-[#121216] border border-gray-800/80 rounded-2xl p-5 shadow-xl glass-card lg:col-span-1">
        ...
        <c:if test="${readOnlyMode}">
            <div class="mt-5 p-4 rounded-xl border border-amber-500/30 bg-amber-950/10">
                <p class="text-amber-300 text-xs font-bold uppercase tracking-wider mb-3"><i class="fas fa-tags mr-1"></i> Quản lý giá vé — cập nhật tiền phụ thu phòng</p>
                <form action="${pageContext.request.contextPath}/admin/rooms/update-room-price" method="POST" class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-end">
                    <input type="hidden" name="id" value="${room.id_Room}">
                    <div class="flex-1">
                        <label class="text-xs text-gray-400 uppercase font-bold tracking-wider mb-1 block">Tiền phụ thu (₫)</label>
                        <input type="number" name="roomPrice" min="0" step="1000" value="${room.roomPrice}" class="w-full bg-[#0a0a0d] border border-amber-600/40 rounded-xl px-4 py-2.5 text-amber-300 font-bold focus:border-amber-500 outline-none">
                    </div>
                    <button type="submit" class="px-5 py-2.5 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-sm uppercase tracking-wider whitespace-nowrap">
                        <i class="fas fa-save mr-1"></i> Cập nhật giá phòng
                    </button>
                </form>
            </div>
        </c:if>

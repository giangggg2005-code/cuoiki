...
<style>
    ...
    .staff-row {
        transition: background-color 0.4s ease, transform 0.35s ease, filter 0.35s ease;
        position: relative;
        z-index: 1;
        cursor: pointer;
    }
    ...
    .staff-row.staff-row-expanded {
        z-index: 80;
        filter: saturate(1.15);
    }
    .staff-row.staff-row-expanded td {
        background: linear-gradient(135deg, rgba(15, 23, 42, 0.96), rgba(30, 41, 59, 0.92), rgba(15, 23, 42, 0.96)) !important;
        padding-top: 1.35rem !important;
        padding-bottom: 1.35rem !important;
        box-shadow: 0 18px 40px -16px rgba(59, 130, 246, 0.55), inset 0 0 22px rgba(59, 130, 246, 0.08);
    }
    .staff-row.staff-row-expanded td:first-child {
        border-top-left-radius: 1rem;
        border-bottom-left-radius: 1rem;
        box-shadow: -3px 0 0 rgba(59, 130, 246, 0.9), 0 18px 40px -16px rgba(59, 130, 246, 0.55), inset 0 0 22px rgba(59, 130, 246, 0.08);
    }
    .staff-row.staff-row-expanded td:last-child {
        border-top-right-radius: 1rem;
        border-bottom-right-radius: 1rem;
        box-shadow: 3px 0 0 rgba(99, 102, 241, 0.9), 0 18px 40px -16px rgba(59, 130, 246, 0.55), inset 0 0 22px rgba(59, 130, 246, 0.08);
    }
    .staff-row.staff-row-expanded .inner-content,
    .staff-row.staff-row-expanded .min-w-0,
    .staff-row.staff-row-expanded .flex.items-center.gap-4 {
    ...
    .staff-row.staff-row-expanded td::before, .staff-row.staff-row-expanded td::after {
        opacity: 1;
    }
    ...
    @keyframes errorToastShimmer {
        0% { transform: translateX(-120%) skewX(-16deg); opacity: 0; }
        22% { opacity: 0.45; }
        100% { transform: translateX(190%) skewX(-16deg); opacity: 0; }
    }
    @keyframes errorToastBorderRun {
        0% { background-position: 0% 50%; }
        100% { background-position: 220% 50%; }
    }
    @keyframes errorToastSoftPulse {
        0%, 100% { box-shadow: 0 14px 28px -18px rgba(248, 113, 113, 0.55), 0 0 0 1px rgba(248, 113, 113, 0.3); }
        50% { box-shadow: 0 18px 36px -16px rgba(248, 113, 113, 0.72), 0 0 0 1px rgba(252, 165, 165, 0.55), 0 0 22px rgba(248, 113, 113, 0.18); }
    }
    ...
    .toast-error-glow {
        ...
        background-image:
            radial-gradient(circle at 12% 20%, rgba(248, 113, 113, 0.18), transparent 30%),
            linear-gradient(135deg, rgba(69, 10, 10, 0.96), rgba(127, 29, 29, 0.94), rgba(69, 10, 10, 0.96));
    }
    .toast-error-glow::before {
        content: '';
        position: absolute;
        inset: 0;
        padding: 1px;
        border-radius: inherit;
        background: linear-gradient(90deg, transparent, rgba(239, 68, 68, 0.75), rgba(254, 202, 202, 0.95), rgba(251, 113, 133, 0.75), transparent);
        background-size: 220% 100%;
        animation: errorToastBorderRun 3s linear infinite;
    ...
    .toast-error-glow::after {
        content: '';
        position: absolute;
        inset: -30% auto -30% -45%;
        width: 30%;
        background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.14), transparent);
        animation: errorToastShimmer 3.4s ease-in-out infinite;
        pointer-events: none;
        z-index: 0;
    }
    .toast-error-glow > * {
    ...
    .field-live-error.hidden {
        display: none;
    }
    .field-has-error {
        border-color: rgba(248, 113, 113, 0.95) !important;
        box-shadow: 0 0 0 1px rgba(248, 113, 113, 0.55), 0 0 14px rgba(239, 68, 68, 0.16) !important;
    }
...
<div id="filterPanel" class="glass-card rounded-2xl mb-6 overflow-hidden transition-all duration-500 ${isFiltering ? 'max-h-[1000px] opacity-100 p-6' : 'max-h-0 opacity-0 p-0'}">
    <form action="${pageContext.request.contextPath}/admin/staffs" method="GET" id="filterForm" onsubmit="return validateFilter()">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4 items-start">
            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all duration-300">
                ...
                <span id="error-filterName" class="field-live-error hidden"></span>
            </div>
            ...
            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all duration-300">
                ...
                <span id="error-filterContact" class="field-live-error hidden"></span>
            </div>

            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-purple-500/30 hover:bg-purple-500/5 transition-all duration-300">
                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Chức Vụ</label>
                <div class="grid grid-cols-1 gap-2">
                    <c:forEach var="role" items="${roles}">
                        <label class="cursor-pointer group">
                            <input type="checkbox" name="roleFilter" value="${role.id_Role}" class="peer hidden" ${not empty roleFilter && roleFilter.contains(role.id_Role) ? 'checked' : ''}>
                            <div class="flex items-center justify-between gap-2 rounded-xl border border-gray-700 bg-[#0d0d0d] px-3 py-2 text-xs text-gray-400 transition-all duration-300 peer-checked:border-purple-500/60 peer-checked:bg-purple-500/10 peer-checked:text-purple-300 group-hover:border-purple-500/30">
                                <span class="inline-flex items-center gap-2 font-bold uppercase tracking-wider">
                ...
                <p class="text-[11px] text-gray-500 mt-2 leading-relaxed">Chọn nhiều quyền để tìm nhân sự có đủ các quyền đã chọn.</p>
            </div>

            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-green-500/30 hover:bg-green-500/5 transition-all duration-300">
            ...
            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-yellow-500/30 hover:bg-yellow-500/5 transition-all duration-300">
                <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Trạng Thái Quyền</label>
                <div class="relative">
                    <i class="fas fa-user-shield absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500"></i>
                    <select name="roleStatusFilter" class="w-full bg-[#0d0d0d] border border-gray-700 text-white text-sm rounded-xl pl-10 pr-4 py-2.5 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none appearance-none transition-all">
                        <option value="ALL" ${roleStatusFilter == 'ALL' ? 'selected' : ''}>Tất cả trạng thái quyền</option>
                        <option value="Active" ${roleStatusFilter == 'Active' ? 'selected' : ''}>Quyền đang Active</option>
                        <option value="Locked" ${roleStatusFilter == 'Locked' ? 'selected' : ''}>Quyền bị Locked</option>
                    </select>
                    <i class="fas fa-chevron-down absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 text-xs pointer-events-none"></i>
                </div>
                <p class="text-[11px] text-gray-500 mt-1 leading-relaxed">Nếu chưa chọn chức vụ, bộ lọc sẽ tìm nhân sự có ít nhất một quyền theo trạng thái này.</p>
            </div>

            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all duration-300">
                ...
                <span id="error-filterStart" class="field-live-error hidden"></span>
            </div>
            ...
            <div class="input-group rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all duration-300">
                ...
                <span id="error-filterEnd" class="field-live-error hidden"></span>
            </div>

            <div class="input-group md:col-span-2 xl:col-span-3 rounded-2xl border border-gray-800/70 bg-black/20 p-3 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all duration-300">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <c:forEach var="s" items="${staffs}">
                            <tr class="staff-row hover:bg-blue-600/5 transition duration-200 group" tabindex="0" title="Bấm để phóng khung nhân viên" onclick="toggleStaffRow(this, event)" onkeydown="handleStaffRowKey(event, this)">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-4 w-64">
                                        ...
                                        <%-- KHỐI CHỨA AVATAR ĐỒNG BỘ LOGIC VỚI TRANG CHỈNH SỬA THÔNG TIN NGƯỜI DÙNG --%>
                                        <div class="avatar-ring shrink-0 shadow-lg transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 relative my-1 ${ringClass}">
                                            <c:choose>
                                                <%-- 1. Nếu avatar có dữ liệu (Link mạng, đường dẫn /assets hoặc tên file) --%>
                                                    ...
                                                    <c:choose>
                                                        ...
                                                        <c:when test="${fn:startsWith(fn:trim(s.user.avatar), 'http')}">
                                                            <img src="${fn:trim(s.user.avatar)}" 
                                                                 class="w-10 h-10 rounded-full object-cover border-2 border-[#161616]"
                                                                 onerror="this.src='${pageContext.request.contextPath}/assets/images/avatar/default-avatar.png';">
                                                        </c:when>
                                                        <%-- TH 1B: Nếu đã là đường dẫn nội bộ /assets/... thì ghép contextPath trực tiếp --%>
                                                        <c:when test="${fn:startsWith(fn:trim(s.user.avatar), '/assets/')}">
                                                            <img src="${pageContext.request.contextPath}${fn:trim(s.user.avatar)}" 
                                                                 class="w-10 h-10 rounded-full object-cover border-2 border-[#161616]"
                                                                 onerror="this.src='${pageContext.request.contextPath}/assets/images/avatar/default-avatar.png';">
                                                        </c:when>
                                                        <%-- TH 1C: Nếu là tên file thì ghép với thư mục avatar của dự án --%>
                                                        <c:otherwise>
                                    ...
                                    <div class="flex flex-col gap-2.5 max-w-[200px]">
                                        <c:choose>
                                            <c:when test="${not empty s.quyen.roleName}">
                                                <c:set var="roleStatusArray" value="${fn:split(s.status, ',')}" />
                                                <c:forEach var="singleRole" items="${fn:split(s.quyen.roleName, ',')}" varStatus="loop">
                                                    <c:set var="trimmedRole" value="${fn:trim(singleRole)}" />
                                                    <c:set var="roleStatusText" value="Active" />
                                                    <c:if test="${fn:length(roleStatusArray) > loop.index}">
                                                        <c:set var="roleStatusText" value="${fn:trim(roleStatusArray[loop.index])}" />
                                                    </c:if>
                                                    ...
                                                    <div class="flex flex-col gap-0.5">
                                                        <div class="flex flex-wrap items-center gap-1.5">
                                                            ...
                                                            <span class="inline-flex items-center gap-1 w-max border px-2 py-0.5 rounded-full text-[9px] uppercase font-black tracking-wider ${roleStatusText == 'Active' ? 'bg-green-500/10 text-green-400 border-green-500/20 shadow-[0_0_8px_rgba(34,197,94,0.12)]' : 'bg-red-500/10 text-red-400 border-red-500/20 shadow-[0_0_8px_rgba(239,68,68,0.12)]'}" title="Trạng thái của quyền ${trimmedRole}">
                                                                <i class="fas ${roleStatusText == 'Active' ? 'fa-circle-check' : 'fa-circle-xmark'} text-[8px]"></i> ${roleStatusText}
                                                            </span>
                                                        </div>
                                ...
                                <td class="px-5 py-4 text-center">
                                    <a href="${pageContext.request.contextPath}/admin/staffs/detail/${s.user.id_User}" onclick="event.stopPropagation()" class="inline-flex w-8 h-8 items-center justify-center bg-gray-800 hover:bg-blue-600 text-gray-400 hover:text-white rounded-lg border border-gray-700 transition duration-200 premium-glow" title="Xem/Sửa chi tiết">
...
<script>
    ...
    function setFieldLiveError(input, errorId, message) {
        const errorTarget = document.getElementById(errorId);
        if (input) input.classList.toggle('field-has-error', !!message);
    ...
    function triggerPremiumToast(type, title, message) {
        ...
        const toastType = String(type || 'error').toLowerCase();
        ...
        toast.innerHTML =
            ...
            '<div class="flex-shrink-0 pl-1">' +
                ...
                '</button>' +
            '</div>' +
            '<div class="bg-gradient-to-r ' + barGradientClass + ' toast-progress-countdown" style="right:0;"></div>';
        toast.style.backgroundColor = toastType === 'success' ? 'rgba(20, 83, 45, 0.96)' : 'rgba(127, 29, 29, 0.96)';
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        const frontendValid = bindRealtimeFilterValidation();

        if (errorMessage) {
            triggerPremiumToast('error', 'Cảnh Báo Hệ Thống', errorMessage);
            return;
        }
        if (successMessage && frontendValid) {
    ...
    function toggleStaffRow(row, event) {
        if (!row) return;
        ...
        const wasExpanded = row.classList.contains('staff-row-expanded');
        document.querySelectorAll('.staff-row.staff-row-expanded').forEach(item => {
            if (item !== row) item.classList.remove('staff-row-expanded');
        });

        row.classList.toggle('staff-row-expanded', !wasExpanded);
    }

    function handleStaffRowKey(event, row) {
        if (!event || !row) return;
        if (event.key === 'Enter' || event.key === ' ') {
            event.preventDefault();
            toggleStaffRow(row, event);
        }
        if (event.key === 'Escape') {
            row.classList.remove('staff-row-expanded');
        }
    }
    ...
    function getFilterValidationElements() {
        return {
            name: document.getElementById('nameKeyword') || document.getElementById('filterName'),
            contact: document.getElementById('contactKeyword') || document.getElementById('filterContact'),
            start: document.getElementById('startDate') || document.getElementById('filterStart'),
            end: document.getElementById('endDate') || document.getElementById('filterEnd')
        };
    }

    function collectFilterValidationErrors() {
        const fields = getFilterValidationElements();
        const nameKeyword = fields.name ? fields.name.value.trim() : "";
        const contactKeyword = fields.contact ? fields.contact.value.trim() : "";
        const startD = fields.start ? fields.start.value : "";
        const endD = fields.end ? fields.end.value : "";
        const errors = [];
        ...
        if (nameKeyword !== "") {
            ...
            if (!nameRegex.test(nameKeyword)) {
                errors.push({
                    field: 'name',
                    title: 'Lỗi Tìm Kiếm',
                    message: 'Họ tên nhân sự tìm kiếm không hợp lệ. Không được chứa số hoặc ký tự đặc biệt như @, #, %, ...'
                });
            }
        }
        ...
        if (contactKeyword !== "") {
            ...
            if (!contactRegex.test(contactKeyword)) {
                errors.push({
                    field: 'contact',
                    title: 'Lỗi Tìm Kiếm',
                    message: 'Thông tin liên hệ không được chứa khoảng trắng hoặc chữ có dấu.'
                });
            }
        }
        ...
        if (startD !== "" && endD !== "") {
            ...
            if (date1 > date2) {
                errors.push({
                    ...
                    message: 'Khoảng ngày lọc không hợp lệ. Thời gian "Từ ngày" không được lớn hơn "Đến ngày".'
                });
            }
        }
    ...
    function renderFilterValidationState(showToast) {
        const state = collectFilterValidationErrors();
        ...
        state.errors.forEach(error => {
            if (error.field === 'name') fieldMessages.name = error.message;
            if (error.field === 'contact') fieldMessages.contact = error.message;
            if (error.field === 'date') {
                fieldMessages.start = error.message;
                fieldMessages.end = error.message;
            }
        });

        setFieldLiveError(state.fields.name, 'error-filterName', fieldMessages.name);
        setFieldLiveError(state.fields.contact, 'error-filterContact', fieldMessages.contact);
        setFieldLiveError(state.fields.start, 'error-filterStart', fieldMessages.start);
        setFieldLiveError(state.fields.end, 'error-filterEnd', fieldMessages.end);

        if (firstError) {
            if (showToast) {
                triggerPremiumToast('error', firstError.title, firstError.message);
            }
            return false;
        }
    ...
    function bindRealtimeFilterValidation() {
        const fields = getFilterValidationElements();
        [fields.name, fields.contact, fields.start, fields.end].forEach(field => {
            if (!field || field.dataset.realtimeValidationBound === 'true') return;
            field.dataset.realtimeValidationBound = 'true';
            field.addEventListener('input', () => renderFilterValidationState(false));
            field.addEventListener('change', () => renderFilterValidationState(false));
            field.addEventListener('blur', () => renderFilterValidationState(false));
        });
        return renderFilterValidationState(false);
    }

    // HÀM VALIDATE ĐỒNG BỘ VỚI FORM onsubmit="return validateFilter()"
    function validateFilter() {
        return renderFilterValidationState(true);
    }

    // Hàm cũ vẫn giữ để tương thích nếu nút nào đang gọi trực tiếp submitFilterForm()

                ...
                <tbody id="showtimeTableBody">
                    <c:if test="${empty showtimes}">
                        <tr>
                            <td colspan="5" class="p-10 text-center text-gray-500 italic">
                                <i class="fas fa-calendar-times text-3xl mb-3 block text-gray-600"></i>
                    ...
                    <c:forEach items="${showtimes}" var="st">
                        <tr class="showtime-row border-b border-gray-800/60 group"
                            data-movie-title="${st.movie != null ? st.movie.title : ''}"
                            data-showtime-status="${st.displayStatus}" 
                            data-room-name="${st.room != null ? st.room.roomName : ''}"
                            data-room-type="${st.room != null && st.room.roomType != null ? st.room.roomType : 'Standard'}"
...
<script>
    ...
    function showToast(message, type) {
        if (typeof triggerPremiumToast === 'function') {
            triggerPremiumToast(type === 'success' ? 'success' : 'error',
    ...
    {
        id: "<c:out value='${r.id_Room}' />",
        roomName: "<c:out value='${r.roomName}' />",
        roomType: "<c:out value='${r.roomType}' default='Standard' />",
        status: "<c:out value='${r.status}' default='Active' />"
    }${!loop.last ? ',' : ''}
    </c:forEach>
    ];
    window.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRoomSelectOptions() {
        ...
        const keywordInput = document.getElementById('roomKeywordSearch');
        if (!listContainer) {
            return;
        }
        ...
        filteredRooms.forEach(room => {
            ...
            statusSpan.className = 'text-[9px] font-mono tracking-wider px-1 py-0.5 rounded bg-gray-800 text-gray-400';
    ...
    // Khi thay đổi trạng thái phòng
    const filterRoomStatusEl = document.getElementById('filterRoomStatus');
    if (filterRoomStatusEl) {
        filterRoomStatusEl.addEventListener('change', function () {
            onRoomStatusChange();
        });
    }

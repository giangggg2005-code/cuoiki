    ...
    <div id="login-container" class="flex w-full max-w-5xl glass-card rounded-[2.5rem] overflow-hidden relative transition-transform duration-300">
        ...
        <div class="flex-1 p-10">
            ...
            <form id="login-form" action="${pageContext.request.contextPath}/login-customer" method="post" class="space-y-6">
                ...
                <div class="mt-6 p-4 bg-green-500/20 border border-green-500/50 rounded-2xl flex items-center space-x-3">
                    <i class="fa-solid fa-circle-check text-green-500"></i>
                    <p class="text-green-100 text-[11px] font-bold uppercase">${successMessage}</p>
                </div>
                </c:if>
                <c:if test="${not empty error}">
                <div id="error-box" class="mt-6 p-4 bg-red-500/20 border border-red-500/50 rounded-2xl flex items-center space-x-3 animate-error">
                    <i class="fa-solid fa-triangle-exclamation text-red-500"></i>
                    <p id="error-text" class="text-red-100 text-[11px] font-bold uppercase">${error}</p>
                </div>
                </c:if>
            </form>

...
<%
    ...
    if (session != null && session.getAttribute("portalRole") != null) {
        portalRole = session.getAttribute("portalRole").toString();
    }
    boolean isManagerPortal = "MANAGER".equalsIgnoreCase(portalRole);
...
<aside id="admin-sidebar" class="w-64 bg-[#121212] border-r border-gray-800/80 fixed left-0 top-16 bottom-0 z-40 p-4 overflow-y-auto sidebar-transition">
    ...
    <div class="space-y-1.5">
        ...
        <a href="${pageContext.request.contextPath}/admin/profile" class="<%= baseClass%> <%= "profile".equals(currentView) ? activeClass : inactiveClass%>">
            <i class="fas fa-user-circle w-5 text-center transition-transform duration-300 group-hover:scale-110 group-hover:text-red-400"></i>

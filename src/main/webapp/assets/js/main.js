...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ========================================================================
    // PROFILE DROPDOWN & LOGOUT CONFIRM (Admin + Staff)
    // ========================================================================
    ...
    window.confirmStaffLogout = function (e) {
    ...
    // Khởi tạo bảng mock chỉ khi trang dùng bảng demo cũ (không phải trang admin/showtimes)
    if (document.getElementById('showtime-table-body') && !document.getElementById('showtimeTableBody')) {
        window.renderShowtimes();
    }
    ...
    window.openEditMovieSystem = function (btn) {
        if (!movieModal || !movieForm || !btn)
            return;
        if (btn.getAttribute('data-status') === 'Closed') {
            return;
        }
        if (frontErrorBox)
            frontErrorBox.classList.add('hidden');

...
<header class="no-print bg-[#121212]/90 border-b border-gray-800/80 px-4 md:px-6 py-3 flex justify-between items-center fixed w-full top-0 z-50 h-16 backdrop-blur">
    ...
    <div class="flex items-center space-x-2 md:space-x-3">
        ...
        <div id="profile-menu-btn" class="relative flex items-center space-x-3 border-l border-gray-800 pl-3 md:pl-4 py-1 cursor-pointer">
            ...
            <%-- Phần Avatar tạo động từ session --%>
            <div class="relative w-9 h-9 rounded-full p-[1.5px] bg-gradient-to-tr from-red-600 to-amber-500 shadow-md">
                ...
                <c:set var="adminAvatar" value="${sessionScope.loggedInUser.avatar}" />
                <c:choose>
                    <c:when test="${not empty adminAvatar && adminAvatar != 'null' && fn:trim(adminAvatar) != ''}">
                        <c:choose>
                            <c:when test="${fn:startsWith(fn:trim(adminAvatar), 'http')}">
                                <c:set var="headerAvatarSrc" value="${fn:trim(adminAvatar)}"/>
                            </c:when>
                            <c:otherwise>
                                <c:set var="headerAvatarSrc" value="${pageContext.request.contextPath}/assets/images/avatar/${fn:trim(adminAvatar)}"/>
                            </c:otherwise>
                        </c:choose>
                    </c:when>
                    <c:otherwise>
                        <c:set var="headerAvatarSrc" value="https://ui-avatars.com/api/?name=${fn:replace(avatarName, ' ', '+')}&background=121212&color=ef4444&bold=true"/>
                    </c:otherwise>
                </c:choose>
                <img src="${headerAvatarSrc}"
                     alt="Avatar"
                     class="w-full h-full object-cover rounded-full bg-[#121212]"
                     onerror="this.src='https://ui-avatars.com/api/?name=${fn:replace(avatarName, ' ', '+')}&background=121212&color=ef4444&bold=true';">
            ...
            <div id="profile-dropdown" class="absolute right-0 top-full mt-2 w-48 bg-[#121212] border border-gray-800 rounded-xl shadow-2xl p-1.5 opacity-0 invisible -translate-y-2 transition-all duration-300 z-50 transform origin-top-right">
                ...
                <a href="${pageContext.request.contextPath}/logout" onclick="return confirmAdminLogout(event)" class="flex items-center space-x-2.5 px-3 py-2 rounded-lg text-xs text-red-400 hover:text-white hover:bg-red-600 transition font-medium">

...
<div class="p-4 sm:p-6 space-y-8">
    ...
    <div class="group/form relative bg-gradient-to-b from-[#1a1a1a] to-[#121212] p-6 sm:p-10 rounded-3xl border border-gray-800/60 shadow-2xl z-20 hover:border-gray-700 transition-all duration-500 mt-12 overflow-hidden">
        ...
        <form id="showtimeForm" action="${pageContext.request.contextPath}/admin/showtimes/save" method="POST" class="space-y-10 relative z-10" onsubmit="return validateForm()">
                    ...
                    <div class="relative group/input" id="customMovieSelectContainer">
                        ...
                        <select name="movieId" id="movieSelect" class="hidden" required>
                            ...
                            <c:forEach items="${movies}" var="m">
                                <option value="${m.id_Movie}" data-duration="${m.duration}" data-price="${m.basePrice}" data-status="${m.status}" ${m.id_Movie == showtime.movieId ? 'selected' : ''}>${m.title}</option>
                            </c:forEach>
                        </select>
            ...
            <div id="showtimePriceManagerPanel" class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gradient-to-br from-amber-950/20 to-black/30 rounded-2xl border border-amber-500/20 shadow-inner">
                <div class="space-y-3">
                    <p class="text-xs font-bold text-amber-400 uppercase tracking-widest"><i class="fas fa-film mr-1"></i> Cập nhật giá phim đang chọn</p>
                    <div id="formUpdateMoviePrice" class="flex gap-3 items-end">
                        <input type="hidden" id="priceMovieId" value="${showtime.movieId}">
                        <div class="flex-1">
                            <label class="text-[10px] text-gray-500 uppercase font-bold mb-1 block">Giá vé cơ bản (₫)</label>
                            <input type="number" id="priceMovieBasePrice" min="0" step="1000" value="0" class="w-full bg-black/50 border border-amber-600/30 rounded-xl px-4 py-3 text-amber-300 font-bold focus:border-amber-500 outline-none">
                        </div>
                        <button type="button" id="btnUpdateMoviePrice" class="px-4 py-3 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-xs uppercase whitespace-nowrap">Cập nhật giá phim</button>
                    </div>
                </div>
                <div class="space-y-3">
                    <p class="text-xs font-bold text-amber-400 uppercase tracking-widest"><i class="fas fa-door-open mr-1"></i> Cập nhật phụ thu phòng đang chọn</p>
                    <div id="formUpdateRoomPrice" class="flex gap-3 items-end">
                        <input type="hidden" id="priceRoomId" value="${showtime.roomId}">
                        <div class="flex-1">
                            <label class="text-[10px] text-gray-500 uppercase font-bold mb-1 block">Tiền phụ thu phòng (₫)</label>
                            <input type="number" id="priceRoomSurcharge" min="0" step="1000" value="0" class="w-full bg-black/50 border border-amber-600/30 rounded-xl px-4 py-3 text-amber-300 font-bold focus:border-amber-500 outline-none">
                        </div>
                        <button type="button" id="btnUpdateRoomPrice" class="px-4 py-3 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-xs uppercase whitespace-nowrap">Cập nhật giá phòng</button>
                    </div>
                </div>
            </div>
...
// BẮT LỖI TỪ BACKEND VÀ HIỂN THỊ LÊN TOAST KHI TẢI TRANG
                            ...
                            function isSelectedMovieClosed() {
                            const movieSelect = document.getElementById('movieSelect');
                            if (!movieSelect || movieSelect.selectedIndex <= 0) return false;
                            const status = movieSelect.options[movieSelect.selectedIndex].getAttribute('data-status');
                            return status === 'Closed';
                            }

                            function refreshMoviePricePanelState() {
                            const closed = isSelectedMovieClosed();
                            const input = document.getElementById('priceMovieBasePrice');
                            const btn = document.getElementById('btnUpdateMoviePrice');
                            if (input) {
                            input.disabled = closed;
                            input.classList.toggle('opacity-50', closed);
                            input.classList.toggle('cursor-not-allowed', closed);
                            }
                            if (btn) {
                            btn.disabled = closed;
                            btn.classList.toggle('opacity-50', closed);
                            btn.classList.toggle('cursor-not-allowed', closed);
                            btn.title = closed ? 'Phim Closed — không thể cập nhật giá' : '';
                            }
                            }

                            function syncPriceManagerPanel() {
                            const priceMovieId = document.getElementById('priceMovieId');
                            if (!priceMovieId) return;
                            const movieSelect = document.getElementById('movieSelect');
                            const roomSelect = document.getElementById('roomSelect');
                            if (movieSelect && movieSelect.value) {
                            priceMovieId.value = movieSelect.value;
                            const mOpt = movieSelect.options[movieSelect.selectedIndex];
                            document.getElementById('priceMovieBasePrice').value = mOpt ? (mOpt.getAttribute('data-price') || 0) : 0;
                            }
                            if (roomSelect && roomSelect.value) {
                            document.getElementById('priceRoomId').value = roomSelect.value;
                            const rOpt = roomSelect.options[roomSelect.selectedIndex];
                            document.getElementById('priceRoomSurcharge').value = rOpt ? (rOpt.getAttribute('data-price') || 0) : 0;
                            }
                            refreshMoviePricePanelState();
                            }

                            function submitInlineMoviePrice() {
                            if (isSelectedMovieClosed()) {
                            triggerPremiumToast('error', 'Lỗi ràng buộc', 'Lỗi ràng buộc: Phim đã ngưng chiếu (Closed)! Không thể cập nhật giá vé cơ bản.');
                            return;
                            }
                            const movieSelect = document.getElementById('movieSelect');
                            if (!movieSelect || !movieSelect.value) {
                            triggerPremiumToast('error', 'Lỗi thao tác', 'Vui lòng chọn phim trước khi cập nhật giá!');
                            return;
                            }
                            const basePrice = document.getElementById('priceMovieBasePrice').value;
                            const fd = new FormData();
                            fd.append('id', movieSelect.value);
                            fd.append('basePrice', basePrice);
                            fd.append('inline', '1');
                            fetch('${pageContext.request.contextPath}/admin/movies/update-base-price', { method: 'POST', body: fd })
                            .then(res => res.json())
                            .then(data => {
                            if (data.success) {
                            const mOpt = movieSelect.options[movieSelect.selectedIndex];
                            if (mOpt) mOpt.setAttribute('data-price', basePrice);
                            updateInitialPrice();
                            triggerPremiumToast('success', 'Thành công', data.message);
                            } else {
                            triggerPremiumToast('error', 'Lỗi hệ thống', data.message || 'Không thể cập nhật giá phim.');
                            }
                            })
                            .catch(() => triggerPremiumToast('error', 'Lỗi hệ thống', 'Không thể cập nhật giá phim.'));
                            }

                            function submitInlineRoomPrice() {
                            const roomSelect = document.getElementById('roomSelect');
                            if (!roomSelect || !roomSelect.value) {
                            triggerPremiumToast('error', 'Lỗi thao tác', 'Vui lòng chọn phòng chiếu trước khi cập nhật giá!');
                            return;
                            }
                            const roomPrice = document.getElementById('priceRoomSurcharge').value;
                            const fd = new FormData();
                            fd.append('id', roomSelect.value);
                            fd.append('roomPrice', roomPrice);
                            fd.append('inline', '1');
                            fetch('${pageContext.request.contextPath}/admin/rooms/update-room-price', { method: 'POST', body: fd })
                            .then(res => res.json())
                            .then(data => {
                            if (data.success) {
                            const rOpt = roomSelect.options[roomSelect.selectedIndex];
                            if (rOpt) rOpt.setAttribute('data-price', roomPrice);
                            updateInitialPrice();
                            triggerPremiumToast('success', 'Thành công', data.message);
                            } else {
                            triggerPremiumToast('error', 'Lỗi hệ thống', data.message || 'Không thể cập nhật giá phòng.');
                            }
                            })
                            .catch(() => triggerPremiumToast('error', 'Lỗi hệ thống', 'Không thể cập nhật giá phòng.'));
                            }

                            const btnUpdateMoviePrice = document.getElementById('btnUpdateMoviePrice');
                            const btnUpdateRoomPrice = document.getElementById('btnUpdateRoomPrice');
                            if (btnUpdateMoviePrice) btnUpdateMoviePrice.addEventListener('click', submitInlineMoviePrice);
                            if (btnUpdateRoomPrice) btnUpdateRoomPrice.addEventListener('click', submitInlineRoomPrice);

                            // Tự động hiện giá vé khi vừa tải trang xong (nếu là trang edit)
                            window.onload = function() { updateInitialPrice(); syncPriceManagerPanel(); }
                            document.getElementById('movieSelect').addEventListener('change', function() { updateInitialPrice(); syncPriceManagerPanel(); });
                            document.getElementById('roomSelect').addEventListener('change', function() { updateInitialPrice(); syncPriceManagerPanel(); });

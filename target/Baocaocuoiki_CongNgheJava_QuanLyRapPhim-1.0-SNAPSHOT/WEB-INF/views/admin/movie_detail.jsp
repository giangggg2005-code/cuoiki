...
<div class="relative min-h-screen p-3 md:p-6 rounded-2xl overflow-hidden bg-[#0a0b0d]">
    ...
    <div class="relative z-10 w-full px-4 lg:px-8">
        ...
        <c:if test="${readOnlyMode && movie.status != 'Closed'}">
            <div class="mb-4 p-4 rounded-xl border border-amber-500/30 bg-amber-950/20 text-amber-300 text-sm font-bold flex items-center gap-2 relative z-10">
                <i class="fas fa-eye"></i> Chế độ xem chỉ đọc — bạn chỉ có thể cập nhật giá vé cơ bản của phim.
            </div>
        </c:if>
        <c:if test="${movie.status == 'Closed'}">
            <div class="mb-4 p-4 rounded-xl border border-gray-500/30 bg-gray-950/30 text-gray-300 text-sm font-bold flex items-center gap-2 relative z-10">
                <i class="fas fa-archive"></i> Phim đã ngưng chiếu (Closed) — chỉ xem thông tin, không thể chỉnh sửa hoặc cập nhật giá.
            </div>
        </c:if>
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            ...
            <div>
                <a href="${pageContext.request.contextPath}${readOnlyMode ? '/admin/showtimes' : '/admin/movies'}" class="glass-panel text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 flex items-center group overflow-hidden relative">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start relative z-10">
            ...
            <div class="lg:col-span-3 flex flex-col items-center lg:items-start">
                ...
                <div class="w-full max-w-[320px] mt-6 space-y-3 relative z-20">
                    ...
                    <c:if test="${readOnlyMode && movie.status != 'Closed'}">
                        <div class="glass-panel rounded-xl p-4 border border-amber-500/20">
                            <p class="text-[10px] text-amber-400 font-bold uppercase tracking-wider mb-2 text-center"><i class="fas fa-tags mr-1"></i> Quản lý giá vé</p>
                            <form action="${pageContext.request.contextPath}/admin/movies/update-base-price" method="POST" class="space-y-3">
                                <input type="hidden" name="id" value="${movie.id_Movie}">
                                <input type="number" name="basePrice" min="0" step="1000" value="${movie.basePrice}" class="w-full bg-black/50 border border-amber-600/40 rounded-xl px-4 py-2.5 text-amber-300 font-bold text-center focus:border-amber-500 outline-none">
                                <button type="submit" class="w-full py-2.5 bg-amber-600 hover:bg-amber-500 text-white font-bold rounded-xl text-xs uppercase tracking-wider">
                                    Cập nhật giá phim
                                </button>
                            </form>
                        </div>
